...
public class ValueTest {
	...
	private void testSetValueWith(int value, int minValue, int maxValue){
		Value toTest = new Value(minValue, minValue, maxValue);
	...
	public void testCloneReflected0(){
		testCloneReflectedWith(0,0,0,10);
	}
	
	@Test
	public void testCloneReflected1(){
		testCloneReflectedWith(0,3,4,7);
	}
	
	@Test
	public void testCloneReflected2(){
		testCloneReflectedWith(3,8,4,9);
	}
	
	@Test
	public void testCloneReflected3(){
		testCloneReflectedWith(4, 4, 4, 4);
	}
	
	
	
	private void testCloneReflectedWith(int minValue, int value, int tempValue, int maxValue){
		Value test = new Value(value, minValue, maxValue);
		test.setTempValue(tempValue);
		List<Field> fields = Arrays.asList(test.getClass().getDeclaredFields());
		assertEquals(4, fields.size()-3); //3 Statische Felder
	...
	private void testEqualsTrueExpectedWith(int minValue, int value, int tempValue, int maxValue){
		Value toTest = new Value(value, minValue, maxValue);
		toTest.setTempValue(tempValue);
		Value another = new Value(value, minValue, maxValue);
	...
	private void testEqualsFalseExpectedWith(int minValue1, int value1, 
		...
		Value first = new Value(value1, minValue1, maxValue1);
		first.setTempValue(tempValue1);
		
		Value second = new Value(value2, minValue2, maxValue2);
